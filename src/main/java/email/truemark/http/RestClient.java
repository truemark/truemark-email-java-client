...
public interface RestClient extends Serializable {
  ...
  /**
   * Sends an HTTP PUT request to issue an update. The response is loaded back
  ...
  /**
   * Sends an HTTP POST request to issue a partial update. The response is loaded back
   * into the given object.
   *
  ...
  /**
   * Sends an HTTP POST request to create a new instance of type T. The response is
   * loaded back into the new object of type T passed into the method.
   *
   * @param uri the URI to POST
   * @param data the request object
   * @param <T> the type of the response object
   * @return the populated response object for convenience
   * @throws IOException if an error occurs
   */
  <T> T postForObject(String uri, Object data, Class<T> clazz) throws IOException;
}
